...
class PoliticoBase(BaseModel):
    nome_civil: str = Field(..., description="Nome oficial do parlamentar", examples=["João Batista da Silva"])
    nome_urna: str = Field(
        ..., description="Nome utilizado na urna (usado na busca global)", examples=["João da Silva"]
    )
    cargo: str = Field(..., examples=["Deputado Federal"])
    partido: str = Field(..., examples=["PL"])
    uf: str = Field(
        ..., min_length=2, max_length=2, description="Sigla do Estado (ex: DF, SP)", examples=["SP"]
    )
...
class ContextoOriginal(BaseModel):
    ...
    texto_extraido: str = Field(..., examples=["Eu sou totalmente a favor de reduzir os impostos da cesta básica..."])
    link_fonte: Optional[str] = Field(
        None, description="Link do YouTube da TV Câmara ou PDF", examples=["https://youtu.be/xyz"]
    )
...
class ResultadoIA(BaseModel):
    ...
    justificativa: Optional[str] = Field(None, examples=["O parlamentar discursou a favor da isenção, mas votou contra o projeto no painel."])
...
class ResultadoSimilaridade(BaseModel):
    ...
    texto_extraido: str = Field(..., examples=["Precisamos taxar as grandes fortunas imediatamente."])

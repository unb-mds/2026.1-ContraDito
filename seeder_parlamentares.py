...
async def buscar_amostra_deputados(quantidade=513):
    async with httpx.AsyncClient(timeout=45.0) as client:
        resposta = await client.get(
            f"{BASE_URL_CAMARA}/deputados", params={"itens": quantidade}
        )
        ...
        tarefas = [
            processar_deputado(client, dep, semaphore, lista_politicos, lista_provas)
...
async def buscar_discursos_senado(client, id_senado_original, qtd_desejada=3):
    try:
        ...
        dados = resp.json()
        discursos_node = (
            dados.get("DiscursosParlamentar", {})
            .get("Parlamentar", {})
            .get("Pronunciamentos", {})
            .get("Pronunciamento", [])
        )
...
async def processar_senador(
    client, senador_data, semaphore, lista_politicos, lista_provas
):
    ...
    async with semaphore:
        try:
            discursos = await buscar_discursos_senado(
                client, id_senado_original, qtd_desejada=3
            )
...
async def buscar_amostra_senadores():
    async with httpx.AsyncClient(timeout=45.0) as client:
        ...
        tarefas = [
            processar_senador(client, dep, semaphore, lista_politicos, lista_provas)
            for dep in lista_basica
        ]
        await asyncio.gather(*tarefas)
...
def salvar_no_supabase(lista_politicos, lista_provas):
    ...
    try:
        resp_politicos = (
            supabase.table("politicos")
            .upsert(lista_politicos, on_conflict="id")
            .execute()
        )
        ...
        resp_provas = (
            ...
            .upsert(lista_provas, on_conflict="politico_id, data_evento, hash_discurso")
            .execute()
        )
...
async def main():
    ...
    resultados_camara, resultados_senado = await asyncio.gather(
        task_camara, task_senado
    )

...
def executar_poc():
    print("Carregando o modelo MPNet (isso pode levar alguns segundos na primeira vez)...")
    ...
    resposta_insert = supabase.table("provas_contradicao").insert(dados_para_inserir).execute()
    ...
    for resultado in resposta_busca.data:
        texto_retorno = resultado.get('texto_extraido', resultado.get('texto', 'Texto não encontrado'))
